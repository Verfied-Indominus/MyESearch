...
def parse_search(search_terms):
    ...
    if result:
        ...
        for res in result:
            ...
            publications.extend([rec.publication for rec in res.pub_tags[:5] if rec.publication not in publications])
    ...
    for n in range(0, length, 2):
        if n+2 <= length:
            ...
            if result:
                ...
                publications.extend([rec.publication for rec in result.pub_records[:15] if rec.publication not in publications])
