...
def parse_search(terms):
    ...
    for key,word in words.items:
        ...
        if word.count(" ") == 1: # definietly a name
            ...
            name = word.split(" ")
            for n in name:
                if Publiation.query.filter_by(first_name=n.capitalize()).first() is not None:
                    name['first_name']= n.capitalize()
                if Publiation.query.filter_by( last_name=n.capitalize()).first() is not None:
        ...
        return search_branch(search_params, content)
